...
function mod_bans_json() {
    ...
    // disabled by default for increased web server compatibility
	...
	if (isset($_POST['board'])) {
		...
		foreach ($replies as &$post) {
			...
			if ($post['has_file']) {
				// copy image
				foreach ($post['files'] as $i => &$file) {
					if ($file['file'] !== 'deleted') //if file exists, process it
						$clone($file['file_path'], sprintf($config['board_path'], $board['uri']) . $config['dir']['img'] . $file['file']);
					if (isset($file['thumb']) && !in_array($file['thumb'], array('spoiler', 'deleted', 'file'))) //if thumb is not spceial, process it's file
						$clone($file['thumb_path'], sprintf($config['board_path'], $board['uri']) . $config['dir']['thumb'] . $file['thumb']);
				}
			}
			// insert reply
	...
	if (isset($_POST['save'])) {
		...
		if (!empty($config_append)) {
			...
			if (!@file_put_contents($config_file, $config_append, FILE_APPEND)) {
				...
				$page['config'] = $config;
				$page['body'] = '
					<p class="text-align-center">Tinyboard could not write to <strong>' . $config_file . '</strong> with the ammended configuration, probably due to a permissions error.</p>
					<p class="text-align-center">You may proceed with these changes manually by copying and pasting the following code to the end of <strong>' . $config_file . '</strong>:</p>
					<textarea class="license" readonly="readonly">' . $config_append . '</textarea>
				';
