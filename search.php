<?php
	...
	if(isset($_GET['search']) && !empty($_GET['search']) && isset($_GET['board']) && in_array($_GET['board'], $boards)) {
		...
		if(!preg_match('/[^*^\s]/', $phrase) && empty($filters)) {
			...
			$body .= '<p class="unimportant text-align-center">(Query too broad.)</p>';
		...
		if($query->rowCount() == $search_limit) {
			_syslog(LOG_WARNING, 'Query too broad.');
			$body .= '<p class="unimportant text-align-center">('._('Query too broad.').')</p>';
			echo Element('page.html', Array(
				'config'=>$config,
				'title'=>'Search',
				'body'=>$body,
		...
		if(!empty($_body))
			...
			$body .= '<p class="unimportant text-align-center">('._('No results.').')</p>';
	}
